...
test('guest kết thúc phiên thì mất quyền truy cập nhưng tin nhắn và canvas vẫn được giữ lại @critical', async ({ page, request }) => {
  ...
  const bootstrap = await request.get(`${e2eApiOrigin}/api/bootstrap`, { headers: originalHeaders });
  ...
  const liveAsset = await request.get(new URL(assetPath!, e2eWebOrigin).toString());
  expect(liveAsset.status()).toBe(200);
  const png = await liveAsset.body();
  expect(png.subarray(1, 4).toString()).toBe('PNG');
  expect(png.readUInt32BE(16)).toBe(1200);
  expect(png.readUInt32BE(20)).toBe(720);
  const assetKey = new URL(assetPath!, e2eWebOrigin).pathname.split('/').pop()!;
  ...
  const continuedCanvasBox = await canvas.boundingBox();
  expect(continuedCanvasBox).not.toBeNull();
  await page.mouse.move(continuedCanvasBox!.x + 80, continuedCanvasBox!.y + 80);
  await page.mouse.down();
  await page.mouse.move(continuedCanvasBox!.x + 180, continuedCanvasBox!.y + 130, { steps: 6 });
  await page.mouse.up();
  await page.getByRole('textbox', { name: 'Lời nhắn cho bản vẽ' }).fill('Canvas tiếp nối');
  const continuationUpload = page.waitForResponse((response) => response.url().includes('/api/assets?room=') && response.request().method() === 'POST');
  await page.getByRole('button', { name: 'Gửi bản vẽ', exact: true }).click();
  expect((await continuationUpload).status()).toBe(200);
  const continuation = page.getByRole('article').filter({ hasText: 'Canvas tiếp nối' });
  await expect(continuation.getByRole('img', { name: 'Bản vẽ phiên bản 2' })).toBeVisible();
  ...
  await expect(page.getByText('Phiên khách đã kết thúc. Bạn không còn quyền truy cập; nội dung đã gửi vẫn ở trong phòng.')).toBeVisible();

  const revokedAssetUrl = await request.get(new URL(assetPath!, e2eWebOrigin).toString());
  expect(revokedAssetUrl.status()).toBe(401);
  const revokedSession = await request.get(`${e2eApiOrigin}/api/rooms/${createdGuest.roomId}/messages`, { headers: originalHeaders });
  expect(revokedSession.status()).toBe(401);

  const observerResponse = await request.post(`${e2eApiOrigin}/api/guest`, { data: { displayName: `Observer ${Date.now()}`, inviteCode } });
  ...
  const retainedHistory = await request.get(`${e2eApiOrigin}/api/rooms/${createdGuest.roomId}/messages`, { headers: observerHeaders });
  ...
  const retainedAsset = await request.get(new URL(retainedCanvas!.assetUrl!, e2eApiOrigin).toString());
  ...
  try {
    ...
    await request.delete(`${e2eApiOrigin}/api/guest`, { headers: observerHeaders }).catch(() => undefined);
...
test('read cursor không đánh dấu tin đến sau lần tải danh sách là đã xem @critical', async ({ request }) => {
  ...
  const guestAResponse = await request.post(`${e2eApiOrigin}/api/guest`, { data: { displayName: `Read A ${stamp}` } });
  ...
  const bootstrapA = await request.get(`${e2eApiOrigin}/api/bootstrap`, { headers: headersA });
  const inviteCode = ((await bootstrapA.json()).rooms as Array<{ inviteCode: string }>)[0].inviteCode;
  const guestBResponse = await request.post(`${e2eApiOrigin}/api/guest`, { data: { displayName: `Read B ${stamp}`, inviteCode } });
  ...
  try {
    const first = await request.post(`${e2eApiOrigin}/api/rooms/${guestA.roomId}/messages`, {
      headers: headersA,
      data: { type: 'text', text: 'Tin đã render' },
    });
    const firstId = ((await first.json()) as { id: string }).id;
    const rendered = await request.get(`${e2eApiOrigin}/api/rooms/${guestA.roomId}/messages`, { headers: headersB });
    expect(((await rendered.json()).messages as Array<{ id: string }>).some((message) => message.id === firstId)).toBe(true);

    const second = await request.post(`${e2eApiOrigin}/api/rooms/${guestA.roomId}/messages`, {
      headers: headersA,
      data: { type: 'text', text: 'Tin đến sau GET' },
    });
    const secondId = ((await second.json()) as { id: string }).id;
    const marked = await request.patch(`${e2eApiOrigin}/api/rooms/${guestA.roomId}/messages`, {
      headers: headersB,
      data: { messageId: firstId },
    });
    expect(marked.status()).toBe(200);

    const after = await request.get(`${e2eApiOrigin}/api/rooms/${guestA.roomId}/messages`, { headers: headersA });
    ...
    await Promise.all([
      request.delete(`${e2eApiOrigin}/api/guest`, { headers: headersA }),
      request.delete(`${e2eApiOrigin}/api/guest`, { headers: headersB }),
    ]);
...
test('kết thúc guest tuần tự hoá với upload và reaction đang chạy @critical', async ({ request }) => {
  ...
  const { db, pool } = createDatabase(databaseUrl, 1);
  const created = await request.post(`${e2eApiOrigin}/api/guest`, {
  ...
  try {
    const sent = await request.post(`${e2eApiOrigin}/api/rooms/${guest.roomId}/messages`, {
      headers,
    ...
    const [reactionResponse, uploadResponse, endResponse] = await Promise.all([
      request.post(`${e2eApiOrigin}/api/messages/${messageId}/reactions`, {
        headers,
        data: { emoji: '✨' },
      }),
      request.post(`${e2eApiOrigin}/api/assets?room=${guest.roomId}`, {
        headers: { ...headers, 'content-type': 'image/png' },
        data: png,
      }),
      request.delete(`${e2eApiOrigin}/api/guest`, { headers }),
    ]);
    ...
    await request.delete(`${e2eApiOrigin}/api/guest`, { headers }).catch(() => undefined);
