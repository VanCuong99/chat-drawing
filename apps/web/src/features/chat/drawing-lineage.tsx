    ...
    <AppDialog open onClose={onClose} labelledBy="lineage-title" describedBy="lineage-description" className="lineage-backdrop">
      <section className="lineage-dialog">
        ...
        {!loading && !error && lineage.length && selected ? <><div className="lineage-scroll-content">
          ...
          {lineage.length > 1 ? <nav className="lineage-filmstrip" aria-label={t('Drawing versions')}>{lineage.map((item) => <button type="button" key={item.id} className={selected.id === item.id ? 'active' : ''} aria-pressed={selected.id === item.id} onClick={() => selectVersion(item)}>{item.assetUrl ? <><img src={item.assetUrl} width="160" height="96" loading="lazy" alt="" /></> : <span className="filmstrip-missing" aria-hidden="true" />}<span><strong>{versionLabel(item)}</strong><small>{item.senderName}</small><em>{item.canvasParentId && byId.get(item.canvasParentId) ? t('From {version}', { version: versionLabel(byId.get(item.canvasParentId)!) }) : t('Starting Point')}</em></span></button>)}</nav> : null}
          <section className="lineage-decision" aria-labelledby="lineage-decision-title">
            <div className="lineage-decision-copy">
              <span className="eyebrow">{t('Team Direction')}</span>
              <h3 id="lineage-decision-title">{selected.visualStatus === 'selected' ? t('Selected Direction') : selected.visualStatus === 'needs_changes' ? t('Needs Another Pass') : t('Still Exploring')}</h3>
              <p>{selected.decisionNote || t('Shortlist this direction or record what the team should do next.')}</p>
              {selected.decidedAt ? <small>{t('Updated {date}', { date: dateFormatter.format(new Date(selected.decidedAt)) })}</small> : null}
            </div>
            <div className="lineage-decision-actions">
              <button type="button" className={selected.voted ? 'vote-button active' : 'vote-button'} aria-pressed={selected.voted} disabled={decisionBusy} onClick={async () => { setDecisionBusy(true); try { await onDecision(selected, { voted: !selected.voted }); } finally { setDecisionBusy(false); } }}>{selected.voted ? t('Shortlisted') : t('Shortlist')} · {selected.voteCount}</button>
              {canDecide ? <button type="button" className="secondary-button" aria-expanded={editingDecision} aria-controls="decision-editor" onClick={() => setEditingDecision((value) => !value)}>{editingDecision ? t('Close Editor') : t('Edit Decision')}</button> : null}
            </div>
            {canDecide && editingDecision ? (
              <div id="decision-editor" className="decision-owner-controls">
                <label className="decision-note-field">
                  <span>{t('Decision Note')}</span>
                  <textarea name="decision-note" autoComplete="off" value={decisionNote} onChange={(event) => setDecisionNote(event.target.value)} maxLength={500} placeholder={t('Why this direction, and what should happen next?')} />
                </label>
                <div className="decision-owner-footer">
                  <div className="decision-owner-fields">
                    <label><span>{t('Status')}</span><select name="visual-status" value={selected.visualStatus} onChange={async (event) => { setDecisionBusy(true); try { await onDecision(selected, { status: event.target.value as CanvasLineageItem['visualStatus'], note: decisionNote }); } finally { setDecisionBusy(false); } }} disabled={decisionBusy}><option value="exploring">{t('Exploring')}</option><option value="needs_changes">{t('Needs Changes')}</option><option value="selected">{t('Selected')}</option></select></label>
                    <label><span>{t('Decision Owner')}</span><select name="decision-owner" value={selected.decisionOwnerId ?? ''} onChange={async (event) => { setDecisionBusy(true); try { await onDecision(selected, { ownerId: event.target.value || null }); } finally { setDecisionBusy(false); } }} disabled={decisionBusy}><option value="">{t('Not Assigned')}</option>{decisionOwners.map((owner) => <option key={owner.id} value={owner.id}>{owner.displayName}</option>)}</select></label>
                  </div>
                  <button type="button" className="primary-button" disabled={decisionBusy} onClick={async () => { setDecisionBusy(true); try { await onDecision(selected, { status: selected.visualStatus, note: decisionNote, ownerId: selected.decisionOwnerId }); setEditingDecision(false); } finally { setDecisionBusy(false); } }}>{t('Save Decision')}</button>
                </div>
              </div>
            ) : null}
          </section>
          </div>
          <footer className="lineage-footer"><div><strong>{t('Create a New Branch')}</strong><small>{t('The selected visual stays unchanged. Your contribution becomes a new branch.')}</small></div><button type="button" className="primary-button" onClick={() => onContinue(selected)} disabled={!selected.assetKey}>{continueLabel}</button></footer>
        </> : null}
      </section>
    </AppDialog>
  );
