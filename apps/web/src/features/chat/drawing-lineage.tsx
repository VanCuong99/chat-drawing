...
function VersionImage({ item, side, mobileActive }: { item: CanvasLineageItem; side: 'A' | 'B'; mobileActive: boolean }) {
  ...
  return (
    <figure className={mobileActive ? 'lineage-compare-card mobile-active' : 'lineage-compare-card'}>
      <figcaption><span>{side}</span><strong>{t('Version {version}', { version: item.canvasVersion ?? 1 })}</strong><small>{item.senderName}</small></figcaption>
      {item.assetUrl ? (
        // The API returns a short-lived, room-scoped URL for each version.
        // eslint-disable-next-line @next/next/no-img-element
        <img src={item.assetUrl} width="1200" height="720" alt={t('Compare {side} · version {version}', { side, version: item.canvasVersion ?? 1 })} />
      ) : <div className="lineage-image-missing">{t('This image is no longer available.')}</div>}
    ...
    <AppDialog open onClose={onClose} labelledBy="lineage-title" describedBy="lineage-description" className="lineage-backdrop">
      <section className="lineage-dialog">
        <header className="lineage-header">
          <div>
            <span className="eyebrow">{t('Drawing Thread')}</span>
            <h2 id="lineage-title" ref={titleRef} tabIndex={-1}>{t('Drawing History')}</h2>
            <p id="lineage-description">{t('Compare changes, then continue from the version that feels right.')}</p>
          </div>
        ...
        {!loading && !error && lineage.length ? (
          <>
            <div className="lineage-count">{t('{count} versions', { count: lineage.length })}</div>
            ...
            {selected ? <footer className="lineage-footer"><div><strong>{t('Continue from version {version}', { version: selected.canvasVersion ?? 1 })}</strong><small>{t('The selected version stays unchanged. Your contribution becomes the next version.')}</small></div><button type="button" className="primary-button" onClick={() => onContinue(selected)} disabled={!selected.assetKey}>{t('Continue from version {version}', { version: selected.canvasVersion ?? 1 })}</button></footer> : null}
          </>
        ) : null}
      </section>
    </AppDialog>
