...
import DrawingStudio from '@/src/features/drawing/drawing-studio';
...
export default function NetApp({ initialUser, initialApiToken, signInPath, signOutPath }: { initialUser: InitialUser; initialApiToken: string | null; signInPath: string; signOutPath: string }) {
  ...
  const selectRoom = useCallback((roomId: string) => {
    ...
    setSidebarOpen(false); setInfoOpen(false); setMessageQuery('');
  }, []);
  ...
  const loadBootstrap = useCallback(async (sessionOverride?: string | null) => {
    ...
    if (data.actor) {
      ...
      if (current !== next) { messageRequestGeneration.current += 1; setMessages([]); setNextCursor(null); setReplyTo(null); }
  ...
  }, [api, guestSession, inviteCode, loadBootstrap]);
  ...
  const loadMessages = useCallback(async (roomId: string, quiet = false, before?: string) => {
    ...
    try {
      ...
      if (activeRoomRef.current !== roomId || generation !== messageRequestGeneration.current) return;
      ...
      if (document.visibilityState === 'visible' && data.messages.length) {
        ...
        if (newest && newest.id !== readMarkers.current.get(roomId)) {
          readMarkers.current.set(roomId, newest.id);
          void api(`/api/rooms/${roomId}/messages`, { method: 'PATCH', body: JSON.stringify({ messageId: newest.id }) })
            .then(() => setRooms((current) => current.map((room) => room.id === roomId ? { ...room, unreadCount: 0 } : room)))
            .catch((readError) => {
              readMarkers.current.delete(roomId);
      ...
      if (!quiet && !before) requestAnimationFrame(() => endRef.current?.scrollIntoView({ block: 'end' }));
    } catch (loadError) {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        socket.on('guest.ended', (payload: { roomId?: string; guestSessionId?: string }) => {
  ...
  const visibleMessages = useMemo(() => normalizedMessageQuery
    ? messages.filter((message) => `${message.senderName} ${message.body ?? ''}`.toLocaleLowerCase('vi').includes(normalizedMessageQuery))
    : messages, [messages, normalizedMessageQuery]);

  if (phase === 'loading') return <div className="boot-screen"><Logo /><span className="loading-line" /><p>Đang mở không gian của bạn…</p></div>;
  ...
  if (phase === 'landing') {
    return (
      <main className="landing-page">
        ...
        {guestModal && (
          <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setGuestModal(false); }}>
            <form className="dialog-card guest-dialog" onSubmit={startGuest}>
              ...
              <span className="eyebrow">Phiên khách</span><h2>{inviteCode ? 'Tham gia cuộc trò chuyện' : 'Bắt đầu một phiên tạm thời'}</h2>
              <p>Dữ liệu bạn tạo sẽ bị xoá khi phiên kết thúc hoặc hết hạn sau 2 giờ không hoạt động.</p>
              <label>Tên hiển thị<input autoFocus value={guestName} onChange={(event) => setGuestName(event.target.value)} placeholder="Ví dụ: Cường" minLength={2} maxLength={60} /></label>
              <label>Mã mời <small>(không bắt buộc)</small><input value={inviteCode} onChange={(event) => setInviteCode(event.target.value)} placeholder="Dán mã hoặc mở link mời" /></label>
  ...
  return (
    <div className="product-root">
      <div className={sidebarOpen ? 'sidebar-scrim show' : 'sidebar-scrim'} onClick={() => setSidebarOpen(false)} />
      <aside className={sidebarOpen ? 'product-sidebar open' : 'product-sidebar'}>
        ...
        <label className="product-search"><span>⌕</span><input value={roomQuery} onChange={(event) => setRoomQuery(event.target.value)} placeholder="Tìm cuộc trò chuyện" aria-label="Tìm cuộc trò chuyện" /></label>
        ...
        <div className="account-card"><span className="avatar" style={avatarStyle(actor?.id ?? 'guest')}>{actor?.displayName.slice(0, 1)}</span><span><strong>{actor?.displayName}</strong><small>{actor?.kind === 'user' ? actor.email : 'Khách · tối đa 2 giờ'}</small></span>{actor?.kind === 'user' ? <a href={signOutPath} title="Đăng xuất" aria-label="Đăng xuất">↗</a> : <button onClick={() => void endGuest()} title="Kết thúc phiên" aria-label="Kết thúc phiên khách">↗</button>}</div>
      </aside>
      ...
      <main className="conversation-panel">
        {activeRoom ? (
          <>
            ...
            {messageQuery !== '' && <div className="message-search"><span>⌕</span><input autoFocus value={messageQuery.trimStart()} onChange={(event) => setMessageQuery(event.target.value || ' ')} placeholder="Tìm nội dung hoặc người gửi" aria-label="Tìm nội dung tin nhắn" /><small>{normalizedMessageQuery ? `${visibleMessages.length} kết quả` : 'Tìm trong cuộc trò chuyện'}</small><button onClick={() => setMessageQuery('')} aria-label="Đóng tìm kiếm">×</button></div>}
            <section className="message-scroll" aria-live="polite">
              <div className="message-lane"><div className="day-pill">Hôm nay</div>
                ...
                {!visibleMessages.length && <div className="conversation-empty"><span>⌁</span><h2>{normalizedMessageQuery ? 'Không tìm thấy tin nhắn' : 'Bắt đầu bằng một lời hoặc một nét'}</h2><p>{normalizedMessageQuery ? 'Thử một từ khoá khác.' : 'Nhắn điều gì đó, gửi ảnh hoặc mở canvas.'}</p></div>}
                      ...
                      <div className="message-content">
                        ...
                        <div id={`message-${message.id}`} className="message-payload">
                          {message.assetUrl && (
                            ...
                            <img src={message.assetUrl} alt={message.type === 'canvas' ? `Bản vẽ phiên bản ${message.canvasVersion ?? 1}` : 'Hình ảnh trong cuộc trò chuyện'} onLoad={() => { if (message.assetKey) automaticAssetRefreshAttempts.current.delete(message.assetKey); endRef.current?.scrollIntoView({ block: 'end' }); }} onError={() => { if (message.assetKey) void refreshAssetUrl(message.assetKey, true); }} />
                          )}
      ...
      {createRoomOpen && <div className="modal-backdrop"><form className="dialog-card" onSubmit={createRoom}><button type="button" className="dialog-close" onClick={() => setCreateRoomOpen(false)} aria-label="Đóng">×</button><span className="eyebrow">Không gian mới</span><h2>Tạo một cuộc trò chuyện</h2><p>Tìm người đã dùng Nét hoặc tạo link để mời thành viên và khách.</p><label>Tìm người bằng tên hoặc email<div className="contact-search"><input autoFocus value={contactQuery} onChange={(event) => setContactQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void searchContacts(); } }} placeholder="Nhập ít nhất 2 ký tự" /><button type="button" onClick={() => void searchContacts()}>Tìm</button></div></label>{selectedContacts.length > 0 && <div className="selected-contacts">{selectedContacts.map((contact) => <button type="button" key={contact.id} onClick={() => setSelectedContacts((current) => current.filter((item) => item.id !== contact.id))}>{contact.displayName} ×</button>)}</div>}{contactResults.length > 0 && <div className="contact-results">{contactResults.map((contact) => <button type="button" key={contact.id} onClick={() => { setSelectedContacts((current) => [...current, contact]); setContactResults((current) => current.filter((item) => item.id !== contact.id)); }}><span className="avatar" style={{ '--avatar': contact.avatarColor } as CSSProperties}>{contact.displayName.slice(0, 1)}</span><span><strong>{contact.displayName}</strong><small>{contact.email}</small></span><b>＋</b></button>)}</div>}<label>Tên cuộc trò chuyện <small>{selectedContacts.length === 1 ? '(có thể để trống)' : ''}</small><input value={roomName} onChange={(event) => setRoomName(event.target.value)} placeholder={selectedContacts.length === 1 ? 'Tự đặt theo hai thành viên' : 'Ví dụ: Góc ban công'} minLength={2} maxLength={60} /></label><label className="checkbox-row"><input type="checkbox" checked={allowGuests} onChange={(event) => setAllowGuests(event.target.checked)} /><span><strong>Cho phép khách tham gia</strong><small>Dữ liệu do khách tạo chỉ tồn tại trong phiên</small></span></label><button className="primary-button wide" disabled={busy || (!roomName.trim() && selectedContacts.length !== 1)}>{busy ? 'Đang tạo…' : selectedContacts.length === 1 ? 'Bắt đầu trò chuyện' : 'Tạo và lấy link mời'}</button></form></div>}
      {studio && <DrawingStudio sourceUrl={studio.sourceUrl} version={studio.version} paletteColors={paletteColors} paletteLoading={paletteLoading} paletteMutating={paletteMutating} onClose={closeStudio} onSend={sendDrawing} onSavePalette={savePaletteColor} onDeletePalette={deletePaletteColor} />}
      {(error || notice) && <div className={error ? 'toast error' : 'toast'} role="status"><span>{error || notice}</span>{error && <button onClick={() => setError('')} aria-label="Đóng thông báo">×</button>}</div>}
    </div>
  );
}
