...
export default function NetApp({ initialUser, initialApiToken, signInPath, signOutPath }: { initialUser: InitialUser; initialApiToken: string | null; signInPath: string; signOutPath: string }) {
    ...
    const boot = window.setTimeout(() => {
      loadBootstrap(savedGuest).then(async (data) => {
        ...
        if (bootstrapError instanceof ApiRequestError && bootstrapError.status === 401 && savedGuest) {
          clearGuestSession('Phiên khách đã hết hạn. Bạn không còn quyền truy cập; hình ảnh và tin nhắn đã gửi vẫn được giữ lại.');
          return;
        }
      ...
      try {
        ...
        socket.on('guest.ended', (payload: { roomId?: string; guestSessionId?: string; messageIds?: string[]; retained?: boolean; removedReactions?: Array<{ messageId: string; emoji: string }> }) => {
          if (actor?.kind === 'guest' && payload.guestSessionId === actorId) {
            ...
            clearGuestSession('');
            setNotice('Phiên khách đã kết thúc. Bạn không còn quyền truy cập; hình ảnh và tin nhắn đã gửi vẫn được giữ lại.');
            return;
          }
  ...
  const startGuest = async (event: FormEvent) => {
    ...
    try {
      const data = await api<{ sessionId: string }>('/api/guest', { method: 'POST', body: JSON.stringify({ displayName, inviteCode: inviteCode || undefined }) }, null);
  ...
  const endGuest = async () => {
    ...
    try {
      ...
      setNotice(result.retained
        ? 'Phiên khách đã kết thúc. Bạn không còn quyền truy cập; hình ảnh và tin nhắn đã gửi vẫn được giữ lại.'
        : 'Phiên khách đã kết thúc.');
    } catch (endError) {
      if (endError instanceof ApiRequestError && endError.status === 401) {
        clearGuestSession('');
        setNotice('Phiên khách đã hết hạn. Bạn không còn quyền truy cập; hình ảnh và tin nhắn đã gửi vẫn được giữ lại.');
      } else {
        endingGuestRef.current = false;
      ...
      <><a className="skip-link" href="#main-content">Bỏ qua đến nội dung chính</a><main id="main-content" className="landing-page">
        ...
        <section className="hero">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><a className="hero-primary" href={signInPath}>Đăng nhập tài khoản <span>→</span></a><button onClick={() => { setError(''); setGuestFormError(''); setGuestErrorField(null); setGuestModal(true); }}>Tiếp tục với tư cách khách</button></div>
        ...
        <AppDialog open={guestModal} onClose={() => { setGuestModal(false); setGuestFormError(''); setGuestErrorField(null); }} labelledBy="guest-dialog-title" describedBy="guest-dialog-description">
            <form className="dialog-card guest-dialog" onSubmit={startGuest} noValidate>
              ...
              <p id="guest-dialog-description">Phiên hết hạn sau 2 giờ không hoạt động. Khi phiên kết thúc, bạn mất quyền truy cập nhưng hình ảnh và tin nhắn đã gửi vẫn được giữ lại.</p>
              ...
              <label>Mã mời <small>(không bắt buộc)</small><input name="invite-code" autoComplete="off" spellCheck={false} value={inviteCode} onChange={(event) => { setInviteCode(event.target.value); setGuestFormError(''); setGuestErrorField(null); }} placeholder="Dán mã hoặc mở link mời…" /></label>
  ...
  return (
    <><a className="skip-link" href="#main-content">Bỏ qua đến nội dung chính</a><div className="product-root">
      ...
      <aside className={sidebarOpen ? 'product-sidebar open' : 'product-sidebar'}>
        <div className="sidebar-head"><Logo compact /><button className="mobile-close" onClick={() => setSidebarOpen(false)} aria-label="Đóng danh sách" data-tooltip="Đóng danh sách" data-tooltip-placement="below">×</button>{actor?.kind === 'user' && <button className="new-room-button" onClick={() => setCreateRoomOpen(true)} aria-label="Tạo cuộc trò chuyện" data-tooltip="Tạo cuộc trò chuyện" data-tooltip-placement="below">＋</button>}</div>
        ...
        <div className="room-list">
          ...
          {!filteredRooms.length && <p className="empty-copy">{rooms.length ? 'Không tìm thấy cuộc trò chuyện.' : 'Chưa có cuộc trò chuyện nào.'}</p>}
        </div>
        {actor?.kind === 'guest' && <div className="guest-retention"><span>Phiên tạm thời</span><p>Bạn mất quyền truy cập khi kết thúc; hình ảnh và tin nhắn đã gửi vẫn được giữ lại.</p></div>}
        <div className="account-card"><span className="avatar" style={avatarStyle(actor?.id ?? 'guest')}>{actor?.displayName.slice(0, 1)}</span><span><strong>{actor?.displayName}</strong><small>{actor?.kind === 'user' ? actor.email : 'Khách · tối đa 2 giờ'}</small></span>{actor?.kind === 'user' ? <a href={signOutPath} aria-label="Đăng xuất" data-tooltip="Đăng xuất" data-tooltip-placement="above">↗</a> : <button className="end-session-button" onClick={() => setGuestEndConfirmOpen(true)} aria-label="Kết thúc phiên khách">Kết thúc phiên</button>}</div>
      </aside>

      <main id="main-content" className="conversation-panel">
        {activeRoom ? (
          <>
            <header className="conversation-header"><button className="mobile-menu" onClick={() => setSidebarOpen(true)} aria-label="Mở danh sách trò chuyện" data-tooltip="Danh sách trò chuyện" data-tooltip-placement="below">☰</button><span className="avatar" style={avatarStyle(activeRoom.name)}>{activeRoom.name.slice(0, 1)}</span><div><strong>{activeRoom.name}</strong><small><i className={realtimeConnected && networkOnline ? '' : 'offline'} /> {realtimeConnected && networkOnline ? 'kết nối trực tiếp' : 'đồng bộ dự phòng'}</small></div><div className="conversation-actions">{installPrompt && <button onClick={() => { void installPrompt.prompt(); setInstallPrompt(null); }} aria-label="Cài ứng dụng" data-tooltip="Cài ứng dụng" data-tooltip-placement="below">⇩</button>}<button onClick={() => setMessageQuery((value) => value ? '' : ' ')} aria-label="Tìm trong tin nhắn" data-tooltip="Tìm tin nhắn" data-tooltip-placement="below">⌕</button><button onClick={() => setInfoOpen((value) => !value)} aria-label="Thông tin cuộc trò chuyện" data-tooltip="Thông tin" data-tooltip-placement="below">i</button></div></header>
            {messageQuery !== '' && <div className="message-search"><span>⌕</span><input name="message-search" autoComplete="off" value={messageQuery.trimStart()} onChange={(event) => setMessageQuery(event.target.value || ' ')} placeholder="Tìm nội dung hoặc người gửi…" aria-label="Tìm nội dung tin nhắn" /><small>{messageSearchLoading ? 'Đang tìm…' : normalizedMessageQuery.length === 1 ? 'Nhập thêm 1 ký tự' : normalizedMessageQuery ? `${messageSearchTotal} kết quả trong toàn bộ lịch sử` : 'Tìm trong cuộc trò chuyện'}</small><button onClick={() => setMessageQuery('')} aria-label="Đóng tìm kiếm" data-tooltip="Đóng tìm kiếm">×</button></div>}
            <section ref={messageScrollRef} className="message-scroll" aria-live="polite" aria-label="Lịch sử tin nhắn">
              <div className="message-lane"><div className="day-pill">Hôm nay</div>
                ...
                {showInviteOnboarding && <section className="invite-onboarding" aria-labelledby="invite-onboarding-title"><span aria-hidden="true">↗</span><div><h2 id="invite-onboarding-title">Mời một người cùng vẽ</h2><p>Sao chép link để người khác vào phòng, nhắn tin và tiếp nối bản vẽ với bạn.</p></div><button className="primary-button" onClick={() => void copyInvite()}>Mời một người</button></section>}
                  ...
                  return (
                    <article key={message.id} className={own ? 'message-row own' : 'message-row'}>
                      ...
                      <div className="message-content">
                        ...
                        <div className="message-tools"><button onClick={() => setReplyTo(message)}>↩ <span>Trả lời</span></button>{message.type === 'canvas' && <button onClick={() => void continueDrawing(message)}>⌁ <span>Vẽ tiếp</span></button>}<div>{EMOJIS.map((emoji) => <button key={emoji} onClick={() => void react(message.id, emoji)} aria-label={`Thả ${emoji}`} data-tooltip={`Thả ${emoji}`}>{emoji}</button>)}</div></div>
                      </div>
                    </article>
                  );
                })}
            ...
            <footer className="composer-zone">
              ...
              <div className="composer"><button onClick={() => fileRef.current?.click()} disabled={busy} aria-label="Chọn ảnh" data-tooltip="Gửi ảnh" data-tooltip-placement="above">＋</button><textarea name="message" autoComplete="off" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void submitText(); } }} placeholder="Nhắn điều gì đó…" maxLength={2000} aria-label="Nội dung tin nhắn" /><button className="draw-button" onClick={() => openStudio({})} disabled={busy} aria-label="Mở canvas" data-tooltip="Mở Studio Nét" data-tooltip-placement="above">⌁</button><button className="send-button" onClick={() => void submitText()} disabled={busy || !draft.trim()} aria-label="Gửi tin nhắn" data-tooltip="Gửi tin nhắn" data-tooltip-placement="above">↑</button></div>
      ...
      <AppDialog open={createRoomOpen} onClose={() => setCreateRoomOpen(false)} labelledBy="create-room-title" describedBy="create-room-description">
        <form className="dialog-card" onSubmit={createRoom}>
          <button type="button" className="dialog-close" onClick={() => setCreateRoomOpen(false)} aria-label="Đóng" data-tooltip="Đóng" data-tooltip-placement="below">×</button>
      ...
      <AppDialog open={guestEndConfirmOpen} onClose={() => setGuestEndConfirmOpen(false)} labelledBy="end-guest-title" describedBy="end-guest-description" className="confirmation-backdrop">
        <section className="dialog-card confirmation-dialog">
          ...
          <p id="end-guest-description">Bạn sẽ mất quyền truy cập ngay. Hình ảnh và tin nhắn đã gửi vẫn được giữ lại trong cuộc trò chuyện.</p>
